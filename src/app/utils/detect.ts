...
const convertOverlayToMask = (overlay: { data: any; }, modelHeight: any, modelWidth: any) => {
  const start = performance.now();
  // @ts-ignore
  const cv = window.cv;
  // Create a Mat from the overlay data
  const mat = cv.matFromArray(modelHeight, modelWidth, cv.CV_8UC4, overlay.data);

  // Convert the overlay from RGBA to grayscale
  let gray = new cv.Mat();
  cv.cvtColor(mat, gray, cv.COLOR_RGBA2GRAY);

  // Apply a binary threshold to convert the grayscale image to a black and white mask
  let mask = new cv.Mat();
 ...
 */
const overflowBoxes = (box: any[], maxSize: number) => {
